...
/*Takes the sourcebyte/firstmsg from the communicator and changes the value of the state changes based on it;
can be used for both noises and music and the message values can be changed*/
void determine_src(int testbyte)
{
    if(testbyte == 0x02)
        gamemode = 1;
    else if(testbyte == 0x04)
        bumper = 1;
    else if(testbyte == 0x05)
        flipper = 1;
    else if(testbyte == 0x06)
        pit = 1;
    else if(testbyte == 0x07)
        wheel = 1;
    else if(testbyte == 0x08)
        hole = 1;
    else if(testbyte == 0x09)
        launcher = 1;
    else if(testbyte == 0x0A)
        scoreboard = 1;
    else if(testbyte == 0x0B)
        wheel = 0;
    else if(testbyte == 0x0C)
        scoreboard = 0;
    else
        wrongbyte = 1;
}

//Takes the msgbyte from the communicator and changes the value of the state changes based on the msgbyte; this may not be necessary
void determine_msg(int testbyte)
{
    if(testbyte == 0x01)
        commonmsg = 1;
    else if(testbyte == 0x02)
        gameover = 1;
    else
        wrongbyte = 1;
}
